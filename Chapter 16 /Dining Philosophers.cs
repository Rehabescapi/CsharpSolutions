...
/***
Program Template made from  https://rosettacode.org/wiki/Dining_philosophers#C.23

Coded by  by Jason Lehmann
...
class Program
{
  private
    const int DinerC = 5;
  private
    static List<Diner> Diners = new List<Diner>();
  private
    static List<Fork> forks = new List<Fork>();
  private
    static DateTime TimeToStop;
  ...
  private
    static string FormatDinerState(Diner diner)
    {
        switch (diner.State)
        {
  ...
  private
    static string FormatForkState(Fork fork)
    {
        return (!ForkIsBeingUsed(fork) ? "  " : "D" + GetForkHolder(fork));
    }

  private
    static bool ForkIsBeingUsed(Fork fork)
    {
            return (Diners.Count(d => d.CurrentlyHeldForks.Contains(fork)) > 0;
    }

  private
    static int GetForkHolder(Fork fork)
    {
        return Diners.Single(d = > d.CurrentHeldForks.Contains(fork)).ID;
    }
}
...
{
  private
    bool IsCurrentlyHoldingLeftFork = false;
  private
    bool IsCurrentlyHoldingRightFork = false;
  private
    const int MaximumWaitTime = 100;
  private
    static Random Randomizer = new Random();
  private
    bool ShouldStopEating = false;
  ...
  public
    IEnumberable<Fork> CurrentlyHeldForks
    {
        get
        {
  ...
  public
    Diner(int id, Fork leftFork, Fork rightFork)
    {
  ...
  private
    void InitializeDinerState(int id, Fork leftFork, Fork rightFork)
    {
        ID = id;
        LeftFork = leftFork;
        RightFork = rightFork;
